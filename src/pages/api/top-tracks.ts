...
import { fetchTopTracks, resolveTrackCoverArtBatch } from "../../lib/lastfm";
...
const CACHE_KEY_PREFIX = "lastfm-top-tracks-v4";
...
export default async function handler(
  ...
  try {
    ...
    // Batch-resolve all cover art in parallel phases (much faster than per-track sequential)
    const coverResults = await resolveTrackCoverArtBatch(
      rawTracks.map((track) => ({
    ...
    const tracks: MusicTrack[] = rawTracks.map((track, index) => ({
      title: track.name,
      artist: track.artist.name,
      cover: coverResults[index],
      songUrl: track.url,
