...
async function fetchWithAuth(url: string, init: RequestInit = {}): Promise<Response> {
  ...
  if (response.status === 401) {
    console.warn(`Spotify API call to ${url} returned 401. Force refreshing token...`);
    const freshToken = await getAccessToken(true);
    const freshHeaders = {
      ...init.headers,
      Authorization: `Bearer ${freshToken.access_token}`,
    };
    response = await fetch(url, { ...init, headers: freshHeaders });
  }
...
export const getNowPlaying = async () => {
  return fetchWithAuth(NOW_PLAYING_ENDPOINT);
};

const QUEUE_ENDPOINT = `https://api.spotify.com/v1/me/player/queue`;

export const getQueue = async () => {
  return fetchWithAuth(QUEUE_ENDPOINT);
};

export async function getTopTracks(timeRange = "short_term") {
  return fetchWithAuth(`${TOP_TRACKS_ENDPOINT}?time_range=${timeRange}&limit=50`);
}
